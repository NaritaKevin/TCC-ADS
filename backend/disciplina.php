...
class Disciplina{
    ...
    public function buscarDados(){
        $res = array();
        $cmd = $this->pdo->query("SELECT * FROM disciplinas");
        $res = $cmd->fetchAll(PDO::FETCH_ASSOC);
        return $res;
    }
    // função para cadastrar pessoas no banco de dados
    public function cadastrarDisciplina($descricao)
    {
        // verificar se o email ja esta cadastrado
        $cmd = $this->pdo->prepare("SELECT disID FROM disciplinas where disDescricao = :disDescricao");
